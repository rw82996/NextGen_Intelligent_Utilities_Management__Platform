 ...
 * via https://github.com/vishalmysore/ragCompressionDemo (Apache-2.0):
...
import { smartCrush, DEFAULT_CONFIG as SC_DEFAULT } from "./smart-crusher";
import { compressLogs, DEFAULT_LOG_CONFIG } from "./log-compressor";
import { compressCode } from "./code-compressor";
import { compressText } from "./text-compressor";
import type { SimConfig, SimResult, DiffChunk } from "../compression-sim";
...
export const METHODS: MethodInfo[] = [
  { key: "none",                label: "No Compression",      desc: "Full context, no reduction",                          category: "baseline" },
  { key: "headroom-smart",      label: "Smart Compress",       desc: "Keyword signals + Kneedle K (~40-60% reduction)",     category: "smart"    },
  { key: "headroom-aggressive", label: "Aggressive Compress",  desc: "Same signals, higher ratio (~70-85% reduction)",      category: "smart"    },
  { key: "tfidf",               label: "TF-IDF",               desc: "Score sentences by query term frequency × IDF",  category: "classic"  },
  { key: "textrank",            label: "TextRank",             desc: "Graph-based importance (PageRank for sentences)",     category: "classic"  },
  { key: "truncation",          label: "Truncation",           desc: "Keep first N sentences — dumb baseline",         category: "baseline" },
  { key: "stopwords",           label: "Stopword Removal",     desc: "Strip \"the/a/is/of…\" — lossless, ~10-20% reduction", category: "advanced" },
  { key: "dedup",               label: "SimHash Dedup",        desc: "Remove near-duplicate sentences via SimHash",         category: "advanced" },
];
...
let _tfidfMod: Promise<typeof import("./tfidf-compressor")> | null = null;
let _textrankMod: Promise<typeof import("./textrank-compressor")> | null = null;
let _extraMod: Promise<typeof import("./extra-compressors")> | null = null;

function loadTfIdf()    { return (_tfidfMod    ??= import("./tfidf-compressor"));    }
function loadTextRank() { return (_textrankMod ??= import("./textrank-compressor")); }
function loadExtra()    { return (_extraMod    ??= import("./extra-compressors"));   }
